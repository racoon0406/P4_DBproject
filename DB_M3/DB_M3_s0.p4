...
parser MyParser(packet_in packet,
    ...
    state parse_pingPong{
        ...
        transition parse_access;
    }

    state parse_access{
        packet.extract(hdr.access);
...
control MyIngress(inout headers hdr,
    ...
    action set_access_Alice() {
        //PUT
        if(hdr.query.queryType == 1)
        {
            //not allowed to write key > 512
            if(hdr.query.key1 > 512)
            {
                hdr.access.no_write_access = 1;
            }
        } 
    }

    action set_access_Bob() {
        //PUT
        if(hdr.query.queryType == 1)
        {
            //not allowed to write key > 256
            if(hdr.query.key1 > 256)
            {
                hdr.access.no_write_access = 1;
            }
        } 
        else if(hdr.query.queryType == 0) //GET
        {
            //not allowed to get key > 256
            if(hdr.query.key1 > 256)
            {
                hdr.access.no_read_access = 1;
            }
        }
        else //RANGE GET
        {
            //not allowed to get if any key in the range > 256
            if(hdr.access.key2_wholeQuery > 256)
            {
                hdr.access.no_read_access = 1;
            }
        }
    ...
    apply {
        if(hdr.ipv4.isValid() && hdr.ipv4.ttl > 0){  
            if(hdr.query.isFeedback == 0 && hdr.pingPong.type == 0) //REQUST
            {
                access_control_lists.apply();               
            }

            if(hdr.access.no_read_access == 1 || hdr.access.no_write_access == 1)
            {
                meta.ecmp_select = 1;
            }
            else
