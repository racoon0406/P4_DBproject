...
def get(key_in, version_in):
    ...
    pkt = pkt / PingPong() / Access(clientID=1) / Query(queryType=0, key1=key_in, version=version_in) / MultiVal() / IP(dst=addr) / TCP(dport=1234, sport=random.randint(49152,65535)) / query_text
    #pkt.show2()
...
def put(key_in, value_in):
    ...
    #print("sending on interface %s to %s" % (iface, str(addr)))
    pkt =  Ether(src=get_if_hwaddr(iface), dst='ff:ff:ff:ff:ff:ff', type=TYPE_PINGPONG)
    pkt = pkt / PingPong() / Access(clientID=1) / Query(queryType=1, key1=key_in, value=value_in) / MultiVal() / IP(dst=addr) / TCP(dport=1234, sport=random.randint(49152,65535)) / query_text
    #pkt.show2()
    sendp(pkt, iface=iface, verbose=False)
...
def range_get(key1_in, key2_in, version_in):
    ...
    for cur_key1 in range(key1_in, key2_in + 1, 10):
        ...
        #print("sending on interface %s to %s" % (iface, str(addr)))
        pkt =  Ether(src=get_if_hwaddr(iface), dst='ff:ff:ff:ff:ff:ff', type=TYPE_PINGPONG)
        pkt = pkt / PingPong() / Access(clientID=1, key1_wholeQuery=key1_in, key2_wholeQuery=key2_in) / Query(queryType=2, key1=cur_key1, key2=cur_key2, version=version_in) / MultiVal() / IP(dst=addr) / TCP(dport=1234, sport=random.randint(49152,65535)) / query_text
        #pkt.show2()
        sendp(pkt, iface=iface, verbose=False)
